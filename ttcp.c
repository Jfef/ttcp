...
char **argv;
	...
	else
	{
		...
		if (trans) // 判断是否是发送端
		{
			while ((cnt = read(0, buf, buflen)) > 0 &&     // 从命令行的标准输入中读取，然后发送掉，重复执行。
...
static void
	prusage(r0, r1, e, b, outp) register struct rusage *r0,
	*r1;
struct timeval *e, *b;
char *outp;
{
...
static void
	tvadd(tsum, t0, t1) struct timeval *tsum,
	*t0, *t1;
{
...
static void
	tvsub(tdiff, t1, t0) struct timeval *tdiff,
	*t1, *t0;
{
